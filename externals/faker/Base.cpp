...
#include "externals/faker/StringVector.h"
#include <chrono>
#include <iostream>
#include <locale>
...
string Base::configDirName =
    "/Users/devloops/Desktop/main_projects/DBMS/sondb/externals/faker/locales";
 ...
 */
void Base::seed(unsigned long val) {
 ...
 *
 * This code may not really work. I've never used std::locale before and the
 * example I'm starting with is stupid. On my Mac, I'm getting a default locale
 * of "C". Yuck.
 */
void Base::setup() {
 ...
 */
long Base::randomNumber(long low, long high) {
  setup();
...
/**
 * This traverses our loaded data along the provided path, loading additional
 * config as appropriate.
 */
Data::Pointer Base::find(const string &path, Data::Vector &dataStack,
                         const std::string &beginningLocale,
...
/**
 * Parse the string. This is the core of the entire library. From Name, we see
 * things like:
 *
 * 		name.name
 * 		name.first_name
 ...
 * 		Under that, look for en.faker.name.name (or first_name, or
 * whatever)
 */
std::string Base::parse(const string &path, const std::string &beginningLocale,
                        const std::string &domain) {
  setup();
