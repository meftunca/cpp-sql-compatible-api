...
#include "externals/faker/Address.h"
#include <iostream>
#include <string>
 ...
 */
std::string Faker::Address::community() { return parse("address.community"); }
 ...
 */
std::string Faker::Address::mail_box() { return parse("address.mail_box"); }

/**
 * Produces a Zip Code.
 *
 * @param state_abbreviation [String] an abbreviation for a state where the zip
 ...
 */
std::string Faker::Address::time_zone() { return parse("address.time_zone"); }
 ...
 */
std::string Faker::Address::state_abbr() { return parse("address.state_abbr"); }
 ...
 */
std::string Faker::Address::state() { return parse("address.state"); }
 ...
 */
std::string Faker::Address::country() { return parse("address.country"); }

/**
 * Produces a country by ISO country code. See the
 * [List of ISO 3166 country
 * codes](https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes) on
...
double Faker::Address::latitude() {
  string degrees =
      std::to_string(randomNumber(-89, 89)) + "." + randomDigits(6);
...
double Faker::Address::longitude() {
  string degrees =
      std::to_string(randomNumber(-179, 179)) + "." + randomDigits(6);
  return std::stod(degrees);
}
