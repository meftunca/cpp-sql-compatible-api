...
#include "externals/faker/Data.h"
 ...
 * Constructor.
 */
Data::Data(Type _type) : type(_type) {}
 ...
 */
void Data::fromJSON(const JSON &json) {
  if (json.is_null()) {
 ...
 */
Data::Pointer Data::find(const string &key) const {
  if (verbose) {
 ...
 * Load a subkey.
 */
Data::Pointer Data::load(const string &path, const string &key) {
  Data::Pointer retVal = nullptr;
  string tryFile = path + "/" + key + ".json";
  struct stat buf;

  // On Mac inside Qt, std::filesystem::exists is problematic, so I'm just
  // going to do this.
...
Data::Pointer Data::makeDomain(const string &key) {
 ...
 */
void Data::loadFile(const string &fileName) {
 ...
 */
void Data::dumpTree(int depth, const string &prefix) const {
  switch (type) {
  case Type::UNKNOWN:
    cout << prefix << "Type Unknown" << endl;
    break;
  case Type::STRING:
    cout << prefix << strValue << endl;
    break;
    ...
    if (depth > 0) {
      for (auto element : map) {
        if (element.second == nullptr) {
          ...
          element.second->dumpTree(depth - 1,
 ...
 * substitution.
 */
std::string Data::expand(Vector &dataStack) const {
  switch (type) {
  case Type::UNKNOWN:
    return "";
  case Type::STRING:
    return expandString(dataStack);
  case Type::ARRAY:
    return expandArray(dataStack);
  case Type::OBJECT:
    return expandObject(dataStack);
  }
 ...
 */
std::string Data::expandString(Vector &dataStack) const {
  string buffer;
  ...
  // This finds all the #{ ... }
  // The funky embedded comments are to balance the braces to help some
  // editors.
  while ((findPos = strValue.find("#{" /*}*/, lastPos)) != string::npos) {
    ...
    if (endPos == string::npos) {
      // Had a start with no more ends, so we'll break to grab the rest of
      // the string.
      break;
    }
    ...
    if (::isupper(mustRecurseOn.at(0))) {
      ...
      std::for_each(mustRecurseOn_LowerCase.begin(),
                    mustRecurseOn_LowerCase.end(),
                    [](char &c) { c = ::tolower(c); });
      ...
      if (stackSize > 0) {
        ...
        for (int index = stackSize - 1; found == nullptr && index >= 0;
             --index) {
...
/**
 * Our example for this is name.name, where we have an array of recursive ways
 * to do it. But we can also support phone_number.formats.
 */
std::string Data::expandArray(Vector &dataStack) const {
 ...
 */
std::string Data::expandObject(Vector &dataStack) const {
 ...
 */
Data::Pointer Data::selectFromArray() const {
 ...
 */
std::string Data::expandFormatString(Vector &) const { return ""; }
...
/**
 * Read file contents. Yes, I know, I could do this differently, but I grabbed
 * this somewhere and I'm trying to remain C++ 11 compatible.
 */
std::string readFileContents(const std::string &path) {
  auto close_file = [](FILE *f) { fclose(f); };

  auto holder = std::unique_ptr<FILE, decltype(close_file)>(
      fopen(path.c_str(), "rb"), close_file);
  if (!holder) {
    return "";
  }
