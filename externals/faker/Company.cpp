...
#include "externals/faker/Company.h"
 ...
 */
std::string Faker::Company::suffix() { return parse("company.suffix"); }
 ...
 */
std::string Faker::Company::industry() { return parse("company.industry"); }
...
std::string Faker::Company::duns_number() {
  return Base::randomDigits(2) + "-" + Base::randomDigits(3) + "-" +
         Base::randomDigits(4);
}
...
std::string Faker::Company::logo() {
  ...
  return std::string(
             "https://pigment.github.io/fake-logos/logos/medium/color/") +
         std::to_string(randomNumber) + ".png";
}
 ...
 */
std::string Faker::Company::type() { return parse("company.type"); }
 ...
 */
std::string Faker::Company::profession() { return parse("company.profession"); }

/**
 * this is a helper method for selecting either one word or a series of
 * space-delimeted words.
 *
 * The path given will feed us (probably) to an array of an array of strings.
 * We'll traverse down from there until we get to the bottom and then randomly
 * select one word from each array.
 */
std::string Faker::Company::selectRandomWords(Data::Pointer top) {
  string retVal = "";

  // This should always be the case, but in case we're used for data I haven't
  // anticipated.
  if (top->getType() == Data::Type::ARRAY) {
    Data::Vector &vec = top->getArray();

    // I don't expect empty arrays.
    // We should be one of two cases. Either we're nested, and we have to
    // dive down (recursively), or we're pointing to an array of strings,
    // and we randomly pick one string.
    ...
    if (vec.size() > 0) {
      ...
      // We're an array of arrays, and have to recurse, once per child
      // array.
      else {
        string delim = "";
...
/**
 * This method is very similar to selectRandomWords(), but we randomly select
 * just ONE.
 */
std::string Faker::Company::selectOneRandomWord(Data::Pointer top) {
  string retVal = "";
